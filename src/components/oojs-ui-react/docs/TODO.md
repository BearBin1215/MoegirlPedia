...
- **ComboBoxInput 缺「点击输入框重开菜单」的展开通道**。原版 `onEdit` 绑定 `keydown mouseup cut paste change input select`，其中 `mouseup` 分支在菜单不可见时会重新展开，即点击已有关键字的输入框就重开菜单（`cut`/`paste`/`select` 由 `input`/`change` 覆盖）。本工程的展开通道只有输入变更、方向键与下拉按钮三处，没有 mouseup 展开通道，表现为「再次点击输入框不重开菜单」。补齐成本低（输入框加 `onMouseUp`，菜单收起时展开）。
- **工具栏有两处未实现**：
...
- **TagMultiselect 未提供合法标签的对外通道**。原版 `getValue()` 只返回合法标签（`items.filter(item => item.isValid())`），调用方拿得到合法子集。本工程的 `value` 就是标签集合本身（`allowDisplayInvalidTags` 开启时含非法项，缺省关闭时非法值不入值），以适应受控语义——若按原版过滤，受控父级回写时会丢失非法标签、显示项与值对不上——但调用方因此无法区分哪些标签非法。内部 `items[].valid` 已算出该信息，仅缺对外通道，补齐成本低。
