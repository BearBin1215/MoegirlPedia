...
$(() => (async () => {
    ...
    class ReminderWindow extends OO.ui.ProcessDialog {
        ...
        async getUserGroup() {
            ...
            let voteTime;
            if (isProposal) {
                voteTime = $(".UTCStartTime").data("utcStartTime");
            } else {
                $(`#${this.sectionTitle.replaceAll(":", "\\:")}`).parents(".discussionContainer").find(".UTCStartTime").data("utcStartTime");
            }
            const userGroup = await api.post({
                action: "query",
                titles: "Module:UserGroup/data",
            ...
            let userList = [];
            for (const item of Object.values(userGroup.query.pages)[0].revisions) {
                if(Date.parse(item.timestamp)/1000 < voteTime) {
                    userList = JSON.parse(item["*"]);
                    break;
                }
            }*/
