...
var _addText = '{{Documentation|content=本工具在提供批量编辑功能，使用者可以在[[Special:MassEdit]]以页面或分类为单位对内容进行批量编辑。\n\n使用：在[[Special:MyPage/common.js|个人js页]]添加如下代码：\n<pre class="prettyprint lang-javascript" style="margin-top:0">mw.loader.load("/index.php?title=User:BearBin/js/MassEdit.js&action=raw&ctype=text/javascript");</pre>\n可搭配[[User:BearBin/js/ListEnhancer.js]]使用。}}';
...
$(() => (async () => {
    ...
    $("#mw-content-text").html([
        ...
        '<li>编辑间隔单位为毫秒（ms），不填默认为0。不包含本身编辑页面所用的时间。</li>',
    ...
    // 获取间隔
    const getInterval = () => Number(intervalBox.getValue());
