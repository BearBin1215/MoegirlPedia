    ...
    if (document.getElementsByClassName("votebox")[0] && (mw.config.get("wgTitle").startsWith("提案/讨论中提案/") || mw.config.get("wgTitle") === "讨论版/权限变更")) {
        ...
        class ReminderWindow extends OO.ui.ProcessDialog {
            ...
            initialize() {
                ...
                this.Sapp = new OO.ui.RadioOptionWidget({ data: "s", label: "管理员" }); // 脚
                this.Iapp = new OO.ui.RadioOptionWidget({ data: "i", label: "管理员、界面管理员" }); // 界
            ...
            async getUserToRemind() {
                ...
                const userExcluded = new Set([...this.userVoted, ...userNoremind]);
                console.log(userToRemind.filter((x) => !userExcluded.has(x)));
        ...
        $(mw.util.addPortletLink("p-cactions", "#", "投票提醒", "ca-vote-remind", GadgetTitle, "r")).on("click", (e) => {
            e.preventDefault();
